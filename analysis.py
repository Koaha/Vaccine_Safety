...
# df = pd.read_csv('synthetic_vaccine_sae_data.csv')
# logger.info(f"Data loaded with shape: {df.shape}")

# # Focus on one vaccine: Rename all to 'TAK' for this analysis
# df['vaccine_1_name'] = 'TAK'
# df['vaccine_2_name'] = df['vaccine_2_name'].fillna('TAK')
# logger.info("Renamed all vaccines to 'TAK' for focused safety analysis.")
...
# # Disproportionality PRR (simplified since one vaccine)
# vacc_ae = pd.crosstab(df['vaccine_1_name'], df['has_severe_AE'])
# prr = (vacc_ae[1] / vacc_ae.sum(axis=1)) / (vacc_ae[1].sum() / vacc_ae.sum().sum())
# logger.info(f"PRR for TAK: {prr}")

# # Save association stats
# prr_summary = f"## PRR for TAK Vaccine\n{prr.to_markdown()}\n\n## Chi-square p-values\n{str(chi2_results)}"
...
# ae_rates_vaccine = df.groupby('vaccine_1_name')['has_severe_AE'].mean().sort_values()
# sns.barplot(x=ae_rates_vaccine.index, y=ae_rates_vaccine.values, palette='viridis')
# plt.title('Severe AE Rates for TAK Vaccine')
# plt.xlabel('Vaccine Name (TAK)')
...
# X-axis: TAK vaccine, y-axis: mean severe AE rate.
# Trends: Single bar for TAK shows overall rate; no comparison needed.
# Observations: Rate around 0.2 suggests moderate risk.
# Conclusions: TAK's AE rate indicates acceptable safety, but monitor for reductions.
...
# X-axis: Outcomes like Full recovery, y-axis: onset time in hours for TAK.
# Trends: Shorter median onset for 'Death', longer for 'Full recovery'.
# Observations: Wide IQR for 'Ongoing', outliers in long onsets.
# Conclusions: Quick onset linked to poorer outcomes in TAK; early intervention key.
...
# # Additional Sankey Plot for symptoms to outcomes
# logger.info("Generating Sankey plot...")
# # Prepare data for Sankey: symptoms to severity levels
# symptoms = ['phu_niem', 'noi_ban', 'kho_tho', 'sot']  # Sample symptoms
# sources = []
# targets = []
# values = []
# labels = symptoms + ['Mild', 'Moderate', 'Severe', 'Fatal']

# for i, sym in enumerate(symptoms):
#     counts = df.groupby(sym)['ae_severity_score'].value_counts().unstack(fill_value=0)
#     for j, level in enumerate([1, 2, 3, 4]):  # Severity levels
#         sources.append(i)
#         targets.append(len(symptoms) + j)
#         values.append(counts.get(level, 0).sum())

# fig = go.Figure(go.Sankey(node=dict(pad=15, thickness=20, line=dict(color="black", width=0.5), label=labels),
#                           link=dict(source=sources, target=targets, value=values)))
# fig.update_layout(title_text="Sankey Diagram: Symptoms to Severity for TAK Vaccine", font_size=10)
# fig.write_html('plots/sankey_plot.html')
# logger.info("Saved sankey_plot.html")
# save_summary_to_md("""
# ![Sankey Plot (Interactive in HTML)](../plots/sankey_plot.html)

# ## Interpretation of Sankey Plot
# Nodes: Left symptoms (e.g., sot), right severity (Mild to Fatal) for TAK.
# Flows: Width proportional to count from symptom to severity.
# Trends: Thicker flows from 'sot' to 'Mild' indicate common mild fever.
# Observations: 'kho_tho' flows more to 'Severe', signaling risk.
# Conclusions: TAK symptoms like breathing issues correlate with severity; prioritize in safety protocols.
# """, 'sankey_plot_interpretation.md')
...
# X = df.drop(['has_severe_AE'] + date_cols + text_cols + ['valid_timing', 'recovery_status', 'ae_severity_score', 'ket_qua'], axis=1, errors='ignore').select_dtypes(include=np.number)
# y = df['has_severe_AE']
...
# X-axis: SHAP value impact, y-axis: Features ordered by importance for TAK.
# Trends: Positive SHAP for 'time_to_onset' means longer onset increases severity prediction.
# Observations: 'age' has mixed impacts, red (high age) pushing negative.
# Conclusions: Key drivers like onset time inform TAK risk models; mitigate high-impact factors.
...
# plt.title('Updated Posterior AE Probability Over Time')
...
# save_summary_to_md("""
# ![Bayesian Update Plot](../plots/bayesian_update.png)

# ## Interpretation of Bayesian Update Plot
# X-axis: Onset dates, y-axis: Posterior AE probability for TAK.
# Trends: Gradual increase then stabilization, with spikes on high-AE days.
# Observations: Probability starts low (~0.001), rises to ~0.15 by end.
# Conclusions: Cumulative evidence shows moderate TAK risk; steady trend suggests consistent safety profile.
...
# plt.title('Kaplan-Meier Survival Curve for Time to Severe AE')
...
# X-axis: Onset time in hours, y-axis: Probability of no severe AE for TAK.
# Trends: Sharp drops early, then plateau, indicating most events within 48 hours.
# Observations: Survival ~0.8 at 24 hours, drops to 0.6 by 72.
# Conclusions: TAK AEs occur quickly; high early survival suggests good short-term safety.
...
# logger.info(f"Overall severe AE rate: {overall_rate:.4f}")
...
# Added Sankey Plot for symptom to outcome
logger.info("Generating Sankey plot...")
symptoms = ['phu_niem', 'noi_ban', 'kho_tho']  # Example symptoms
outcomes = df['ket_qua'].unique()
...
for i, sym in enumerate(symptoms):
    for j, out in enumerate(outcomes):
        count = df[(df[sym] == 1) & (df['ket_qua'] == out)].shape[0]
        if count > 0:
            source.append(symptom_indices[i])
            target.append(outcome_indices[j])
            value.append(count)

labels = symptoms + list(outcomes)
...
save_summary_to_md("""
# Sankey Diagram Report for TAK Vaccine Safety

![Sankey Diagram](../plots/sankey_symptom_outcome.html) (Open in browser for interactivity)

## Detailed Interpretation
**Nodes:** Left: Symptoms (e.g., phu_niem), Right: Outcomes (e.g., Full recovery).
**Flows:** Thickness proportional to case count from symptom to outcome.
**Trends Observed:** Thicker flows from certain symptoms to poor outcomes highlight risks.
**Conclusions:** For TAK, if 'kho_tho' flows heavily to 'Ongoing', it concludes breathing issues lead to prolonged recovery, observing trends for symptom-specific safety.
